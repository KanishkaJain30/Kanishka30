...
from configuration import CONSUMER_KEY, CONSUMER_SECRET
from configuration import TWEETS_SINCE_DAYS, TWEETS_COUNT, TOP_COUNT
from configuration import CALLBACK_URL
...
def top_tweets_list(request):
    ...
    tweets_top_urls = Tweet.objects.all().\
                            values('user_id', 'user_handle_name', 'user_name').\
                            annotate(total=Count('tweet_url')).\
